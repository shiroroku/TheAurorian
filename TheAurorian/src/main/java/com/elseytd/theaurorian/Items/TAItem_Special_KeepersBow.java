...
public class TAItem_Special_KeepersBow extends ItemBow {

	public static final String ITEMNAME = "keepersbow";

	public TAItem_Special_KeepersBow() {
		...
		this.addPropertyOverride(new ResourceLocation("pull"), new IItemPropertyGetter() {
			...
			public float apply(ItemStack stack, @Nullable World worldIn, @Nullable EntityLivingBase entityIn) {
				if (entityIn == null) {
					...
					return entityIn.getActiveItemStack().getItem() != TAItems.Registry.KEEPERSBOW.getItem() ? 0.0F : (stack.getMaxItemUseDuration() - entityIn.getItemInUseCount()) / 20.0F;
				}
			}
		});
		...
		if (entityLiving instanceof EntityPlayer) {
			...
			if (!itemstack.isEmpty() || flag) {
				...
				if (f >= 0.1D) {
					...
					worldIn.playSound((EntityPlayer) null, entityplayer.posX, entityplayer.posY, entityplayer.posZ, SoundEvents.ENTITY_ARROW_SHOOT, SoundCategory.PLAYERS, 1.0F, 1.0F / (itemRand.nextFloat() * 0.4F + 1.2F) + f * 1F);
