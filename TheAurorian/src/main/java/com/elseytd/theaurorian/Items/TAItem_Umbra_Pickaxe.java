...
public class TAItem_Umbra_Pickaxe extends ItemPickaxe {
	...
	public static void renderBlockOutline(DrawBlockHighlightEvent e) {
		...
		if (player.getHeldItemMainhand().getItem() instanceof TAItem_Umbra_Pickaxe) {
			ItemStack stack = player.getHeldItemMainhand();
			TAItem_Umbra_Pickaxe pick = (TAItem_Umbra_Pickaxe) stack.getItem();
		...
		if (e.getGui() instanceof GuiContainer) {
			...
			for (Slot s : container.inventorySlots.inventorySlots) {
				if (!s.getStack().isEmpty()) {
					...
					if (stack.getItem() instanceof TAItem_Umbra_Pickaxe) {
	...
	public float getDestroySpeed(ItemStack stack, IBlockState state) {
		if (getSelectedBlock(stack) != null) {
			if (state.getBlock() == getSelectedBlock(stack)) {
				return super.getDestroySpeed(stack, state) * TAConfig.Config_UmbraPickaxeMiningSpeedMultiplier;
			}
		}
