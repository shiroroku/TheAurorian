...
public class TAMapGenCaves extends MapGenBase {
	...
	protected boolean canReplaceBlock(IBlockState p_175793_1_, IBlockState p_175793_2_) {
		if (p_175793_1_.getBlock() == TABlocks.aurorianstone) {
			return true;
		} else if (p_175793_1_.getBlock() == TABlocks.auroriandirt) {
			return true;
		} else if (p_175793_1_.getBlock() == TABlocks.auroriangrass) {
			return true;
		} else {
	...
	private boolean isTopBlock(ChunkPrimer data, int x, int y, int z, int chunkX, int chunkZ) {
		...
		return (isExceptionBiome(biome) ? state.getBlock() == TABlocks.auroriangrass : state.getBlock() == biome.topBlock);
	}
	 ...
	 * @param foundTop True if we've encountered the biome's top block. Ideally
	 *                 if we've broken the surface.
	 */
