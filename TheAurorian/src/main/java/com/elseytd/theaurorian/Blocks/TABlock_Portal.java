...
public class TABlock_Portal extends BlockBreakable {
	...
	public boolean trySpawnPortal(World worldIn, BlockPos pos) {
		...
		if (blockportal$size.isValid() && blockportal$size.portalBlockCount == 0) {
			...
			TABlock_Portal.Size blockportal$size1 = new TABlock_Portal.Size(worldIn, pos, EnumFacing.Axis.Z);
			...
			if (entityIn instanceof EntityPlayer) {
				if (entityIn.timeUntilPortal > 0) {
					...
					if (entityIn.dimension == 0) {
						...
						TATeleporter.transferEntity(entityIn, 0);
					}
				}
			}
		}
	...
	public BlockPattern.PatternHelper createPatternHelper(World worldIn, BlockPos p_181089_2_) {
		...
		TABlock_Portal.Size blockportal$size = new TABlock_Portal.Size(worldIn, p_181089_2_, EnumFacing.Axis.X);
		LoadingCache<BlockPos, BlockWorldState> loadingcache = BlockPattern.createLoadingCache(worldIn, true);

		if (!blockportal$size.isValid()) {
			enumfacing$axis = EnumFacing.Axis.X;
			blockportal$size = new TABlock_Portal.Size(worldIn, p_181089_2_, EnumFacing.Axis.Z);
		}
