...
import com.elseytd.theaurorian.Items.SickleItem;
...
public class SilkberryBlock extends BlockBush implements IShearable {
	...
	public void getDrops(NonNullList<ItemStack> drops, IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
		...
		ItemStack seed = new ItemStack(ItemRegistry.Registry.SILKBERRY.getItem());
		if (!seed.isEmpty()) {
			drops.add(seed);
		}
	...
	public List<ItemStack> onSheared(ItemStack item, IBlockAccess world, BlockPos pos, int fortune) {
		if (item.getItem() instanceof SickleItem) {
			return NonNullList.withSize(1, new ItemStack(ItemRegistry.Registry.SILKBERRY.getItem()));
		} else {
